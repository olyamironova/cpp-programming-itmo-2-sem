...
namespace postorder {
	...
	public:
		...
		iterator Begin() {
			return iterator(SearchMinimum());
		}

		/* Версия begin() для const */
		const_iterator Begin() const {
			return const_iterator(SearchMinimum());
		}
		...
		struct BinarySearchTreeNode {
			...
			* значением key */
			...
			tree_node* NextNode() const {
				...
				if (parent->right_ == nullptr || parent->right_ == node) {
					return parent;
				}
				tree_node* current = parent->right_;
			...
			tree_node* PrevNode() const {
				...
				tree_node* parent = temp_node->parent_;
				while ((parent != nullptr) && (parent->left_ = temp_node)) {
