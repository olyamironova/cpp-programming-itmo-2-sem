...
namespace preorder {
	...
	public:
		...
		iterator End() {
			if (head_->right_ == nullptr) {
				return iterator(head_);
			}
			else {
				tree_node* node = head_;
				while (node != nullptr) {
				...
				tree_node* imaginary_node;
				node.right_ = imaginary_node;
				imaginary_node->parent_ = node;
				return iterator(imaginary_node);
			}
		}
		...
		const_iterator End() const {
			if (head_->right_ == nullptr) {
				...
				node.right_ = imaginary_node;
				imaginary_node->parent_ = node;
				return const_iterator(imaginary_node);
			}
		}
		...
		struct BinarySearchTreeNode {
			...
			* значением key */
			...
			tree_node* NextNode() const {
				...
				tree_node* ancestor = node->parent_;

				while (ancestor) {
					if (ancestor->left_ == node) {
						successor = ancestor->right_;
						break;
					}
					if ((ancestor->parent_) && (ancestor->parent_->left_ == ancestor) && (ancestor->parent_->right_)) {
						successor = ancestor->parent_->right_;
						break;
					}
					node = ancestor;
			...
			tree_node* PrevNode() const {
				...
				tree_node* current = parent->left_;
